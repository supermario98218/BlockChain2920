        ...
        def actualOperation(self):
            ...
            while (self.counter2 < 12):
                if self.counter2 == 0:
                    ...
                    url = "https://www.wunderground.com/hourly/us/ga/athens/date/2018-04-18?cm_ven=localwx_hour"
                    ...
                    print(webValue, " This value is extracted during each iteration.  But since period so small, there will be small change in web-source predicted outside temp.")
                    print("User boundaries: +/- 3.7 degrees F" )
                    
                    if((self.webChain[self.counter2].getData() == self.trueChain[self.counter2].getData())  or  ((self.webChain[self.counter2].getData() > self.trueChain[self.counter2].getData()) and (self.webChain[self.counter2].getData() <= (self.trueChain[self.counter2].getData() +3.7)))  or ((self.webChain[self.counter2].getData() < self.trueChain[self.counter2].getData()) and (self.webChain[self.counter2].getData() >= (self.trueChain[self.counter2].getData() -3.7))) ):
                            self.trueChain[self.counter2].setData(webValue)
                            self.webValid[self.counter2] = True
                            print("web-source within bounds - if")
                    #ends inner if
                    self.howFarFromTrue[self.counter2] = abs(round((webValue - f),2)) #f is also self.trueChain[self.counter2].getData().  Latter used in conditionals to show proof of blockchain comparison understanding
                #ends outer if
                else:
                    h,t = dht.read_retry(dht.DHT22, 4)
                    f = round ((9.0/5.0*t+32), 2)
